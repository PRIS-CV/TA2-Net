...
class CheckPointer(object):
        ...
        if os.path.isfile(ckpt_path):
          #  print("=> loading {} checkpoint '{}'".format(ckpt, ckpt_path))
            ...
            if self.model is not None and model:
                ...
                if 'backbone' in list(model_dict.keys())[0]: 
                    pretrained_dict = {'backbone.' + k: v for k, v in ch['state_dict'].items()} 
                    
                    self.model.load_state_dict(pretrained_dict, strict=strict)
                else:
    ...
    def save_checkpoint(self, epoch, best_val_acc, best_val_loss,
                        is_best, filename='meta_checkpoint.pth.tar',
        ...
        if is_best:
            shutil.copyfile(os.path.join(model_path, filename),
                            os.path.join(model_path, 'meta_model_best.pth.tar'))

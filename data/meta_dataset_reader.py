...
ALL_METADATASET_NAMES_1 = "traffic_sign".split(' ')

#ALL_METADATASET_NAMES = "vgg_flower dtd ilsvrc_2012 cu_birds omniglot fungi aircraft quickdraw cifar100 mscoco mnist traffic_sign cifar10".split(' ')
#TRAIN_METADATASET_NAMES = ALL_METADATASET_NAMES[:8]
#TEST_METADATASET_NAMES = ALL_METADATASET_NAMES[8:]

ALL_METADATASET_NAMES = "mscoco cifar100 cifar10 traffic_sign mnist quickdraw omniglot aircraft dtd vgg_flower fungi cu_birds ilsvrc_2012".split(' ')
TRAIN_METADATASET_NAMES = ALL_METADATASET_NAMES[5:]
...
class MetaDatasetEpisodeReader(MetaDatasetReader):
    ...
    """
    def __init__(self, mode, train_set=None, validation_set=None, test_set=None, test_type='1shot'):
        ...
        # varying-way-five-shot
            ...
            if mode == 'train':
                train_episode_desscription = config.EpisodeDescriptionConfig(None, 5, None,  min_ways=5, max_ways_upper_bound=5, max_num_query=10,max_support_size_contrib_per_class=5)
                self.train_dataset_next_task = self._init_multi_source_dataset(
                    train_set, SPLIT_NAME_TO_SPLIT['train'], train_episode_desscription)
...
class MetaDatasetBatchReader(MetaDatasetReader):
    ...
    def _init_multi_source_dataset(self, items, split):
        ...
        if items == ['ilsvrc_2012']:
            data_images_per_class = list(dataset_specs[0][2].values())[0]
            
            
        else: 
            data_images_per_class = dataset_specs[0][2]
        for value in data_images_per_class.values():
            
            self.value_all = self.value_all + value
